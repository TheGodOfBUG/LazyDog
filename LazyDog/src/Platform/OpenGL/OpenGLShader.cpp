...
namespace LazyDog
{
	...
	std::string OpenGLShader::ReadFile(const std::string& filepath)
	{
		...
		{
			LD_CORE_ERROR("Could not open file '{0}'", filepath);
		}
		return result;
	}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			LD_CORE_ASSERT(type == "vertex" || type == "fragment" || type == "pixel", "Invalid shader type specified!");
	...
	void OpenGLShader::Compile(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		LD_CORE_ASSERT(shaderSources.size() <= 2, "We only support 2 shaders for now (vertex and fragment)");
		...
		if (isLinked == GL_FALSE)
		{
			...
			LD_CORE_ERROR("{0}", infoLog.data());
			LD_CORE_ASSERT(false, "Shader link failure!");
			return;
		}
